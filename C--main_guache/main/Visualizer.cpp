...
std::string Visualizer::buildInfoString(const Vehicle& egoVehicle, double time, const EudmManager& manager) const {
    ...
    std::string currentActionStr = "Current actions: [" + currentAction.lon + "," + currentAction.lat + "]";

    std::string decisionStr = "Decision sequence: ";
    ...
    ss << "Time: " << time << "s\n"
       << "Speed: " << egoVehicle.vy * 3.6 << " km/h\n"
       << "Lane: " << currentLane << "\n"
       << "Course angle: " << (egoVehicle.theta * 180.0 / M_PI) << "°\n"
       << "Location: (" << egoVehicle.x << ", " << egoVehicle.y << ")m\n"
       << currentActionStr << "\n"
       << decisionStr;
    ...
    if (stats.planCount > 0) {
        ss << "\n\nPlanning and Statistics:\n"
           << "Number of plans: " << stats.planCount << "\n"
           << "average time: " << stats.avgPlanTime * 1000.0 << "ms\n"
           << "Last time: " << stats.lastPlanTime * 1000.0 << "ms";
    }
    return ss.str();
}
...
void Visualizer::draw(const Vehicle& egoVehicle,
    ...
    for(size_t i=0; i<obstacleVehicles.size(); ++i) {
        ...
        // 3a. 绘制障碍物挂车
        if (obs.hasTrailer) {
            sf::VertexArray obsTrailerShape = createRotatedShape(obs.getTrailerShape());
            for (int j = 0; j < 4; ++j) obsTrailerShape[j].color = fillColor;
            window_.draw(obsTrailerShape);
        }
    ...
    // 4a. 绘制自车挂车
    if (egoVehicle.hasTrailer) {
        sf::VertexArray egoTrailerShape = createRotatedShape(egoVehicle.getTrailerShape());
        for (int j = 0; j < 4; ++j) {
            egoTrailerShape[j].color = EGO_COLOR;
        }
        window_.draw(egoTrailerShape);
    }
    // 自车标签
    if(font_->getInfo().family != "") {
