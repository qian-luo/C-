...
std::pair<double, double> EudmManager::actionToControl(const Action& action,
    ...
    allVehicles.push_back(egoVehicle);  // 先加自车
    allVehicles.insert(allVehicles.end(), obstacleVehicles.begin(), obstacleVehicles.end()); // 再加障碍物
    ...
    // 方案1：给debugInfo_加一个初始化标记（推荐）
if (debugInfo_.isLastSteerInitialized) { // 新增bool成员变量，默认false
    double maxSteerRate = M_PI / 6.0;
    ...
    debugInfo_.isLastSteerInitialized = true; // 首次运行标记为已初始化
}
debugInfo_.lastSteer = steer; // 无条件更新，和MATLAB一致
...
int EudmManager::getTargetLaneFromAction(const Action& action,
    ...
    int currentLane = road.getLaneIndex(egoVehicle.x);
    const int totalLanes = road.getNumLanes(); // 总车道数，和MATLAB road.numLanes一致
    
    if (action.lat == "LCL") {
        // 左变道：下限1，完全匹配MATLAB max(1, ...)
        return std::max(1, currentLane - 1);
    } else if (action.lat == "LCR") {
        // 右变道：上限totalLanes，完全匹配MATLAB min(road.numLanes, ...)
        return std::min(totalLanes, currentLane + 1);
    } else {
        return currentLane;
    }
}
