...
public:
    ...
    simulateLayer(const Action& action, Vehicle& egoVehicle, 
                  std::vector<Vehicle>& obstacleVehicles, const Road& road)const;

    bool checkRSSSafety(const Vehicle& ego, const std::vector<Vehicle>& obs, const Road& road)const;

    bool checkLaneChangeSafety(const Action& action, const Vehicle& ego, 
                              const std::vector<Vehicle>& obstacles, const Road& road)const;

    double calculateRSSSafeLongitudinalDistance(double v_rear, double v_front, bool is_front)const;
    ...
    const Vehicle* findLeadingVehicle(const Vehicle& current, const Road& road, const std::vector<Vehicle>& allVehicles) const;

    double calculateIDM(const Vehicle& ego, const Vehicle* leader) const;
